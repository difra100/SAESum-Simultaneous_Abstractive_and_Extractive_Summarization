...
class MemSum:
    def __init__( self, model_path, vocabulary_path, gpu = None , embed_dim=200, num_heads=8, hidden_dim = 1024, N_enc_l = 2 , N_enc_g = 2, N_dec = 3,  max_seq_len =100, max_doc_len = 500, pegasus_mode = False  ):
        ...
        self.local_sentence_encoder = LocalSentenceEncoder_MemSum_Full( vocab_size, self.vocab.pad_index, embed_dim,num_heads,hidden_dim,N_enc_l, None, pegasus_mode=pegasus_mode)
    ...
    def extract( self, document_batch, p_stop_thres = 0.7, ngram_blocking = False, ngram = 3, return_sentence_position = False, return_sentence_score_history = False, max_extracted_sentences_per_document = 4 ):
        ...
        for document in tokenized_document_batch:
            ...
            if self.peg:
                in_seqs = tokenize_sentences(
                document, peg_tokenizer, self.max_seq_len)
                input_ids = torch.cat((input_ids, torch.from_numpy(in_seqs['input_ids']).to(self.device)), dim = 0)
                attention_mask = torch.cat((attention_mask, torch.from_numpy(in_seqs['attention_mask']).to(self.device)), dim = 0)
        ...
        with torch.no_grad():
            ...
            if self.peg:
                ...
                # print(in_seqs['input_ids'])
                num_sentences = in_seqs['input_ids'].shape[1]
                ...
                sen_embed  = self.local_sentence_encoder( seqs)
