...
class LocalSentenceEncoder( nn.Module ):
    ...
    def forward( self, input_seq, dropout_rate = 0. ):
        ...
        #pa
        if not self.peg:
            B, D, S = input_seq.shape
            mask = self.addmask(input_seq.view(-1, S))
            net = self.word_embedding[input_seq.view(-1, S)]
        else:
            B, D, S = input_seq['input_ids'].shape
            mask = self.addmask(input_seq['input_ids'].view(-1, S))
            input_seq['input_ids'] = input_seq['input_ids'].view(-1, D*S)
            input_seq['attention_mask'] = input_seq['attention_mask'].view(-1, D*S)
            net = get_enc_output(input_seq).detach()
            emb_dim = net.shape[-1]
            # print("EMBEDDINGS Shape: ", net.shape)
            net = net.reshape(B, D, S,-1)
            # print("first reshape: ", net.shape)

            net = net.reshape(B*D, S, emb_dim)

            
        # print("After reshape NET: ", net.shape)
        net, _ = self.rnn( net )
...
#pa
def get_enc_output(token_ids, detach = True):
  ''' This function takes as input the tokens produced by the pegasus tokenizer '''

  return model_s.model.encoder(**token_ids).last_hidden_state