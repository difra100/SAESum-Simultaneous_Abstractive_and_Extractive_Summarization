...
def update_moving_average(m_ema, m, decay):
    with torch.no_grad():
        param_dict_m_ema = m_ema.module.parameters() if isinstance(
            m_ema, nn.DataParallel) else m_ema.parameters()
        param_dict_m = m.module.parameters() if isinstance(
            m, nn.DataParallel) else m.parameters()
...
# Step 3: Load the Pegasus model and tokenizer (pa)
...
local_sentence_encoder = LocalSentenceEncoder(
    vocab_size, vocab.pad_index, embed_dim, num_heads, hidden_dim, N_enc_l, pretrained_embedding, pegasus_mode=pegasus_mode)
global_context_encoder = GlobalContextEncoder(
    embed_dim, num_heads, hidden_dim, N_enc_g)
...
device = torch.device("cuda:%d" % (
    gpu_list[0]) if torch.cuda.is_available() else "cpu")
...
extraction_context_decoder_ema = copy.deepcopy(
...
if device.type == "cuda" and n_device > 1:
    ...
    extraction_context_decoder = nn.DataParallel(
        extraction_context_decoder, gpu_list)
    extractor = nn.DataParallel(extractor, gpu_list)

    local_sentence_encoder_ema = nn.DataParallel(
        local_sentence_encoder_ema, gpu_list)
    global_context_encoder_ema = nn.DataParallel(
        global_context_encoder_ema, gpu_list)
    extraction_context_decoder_ema = nn.DataParallel(
        extraction_context_decoder_ema, gpu_list)
...
if ckpt is not None:
    ...
    current_epoch = int(current_batch * batch_size_per_device *
                        n_device / total_number_of_samples)
...
def train_iteration(batch):
    ...
    if pegasus_mode:
        ...
        num_documents = batch['input_ids'].size(0)
        num_sentences = batch['input_ids'].size(1)
    ...
    local_sen_embed = local_sentence_encoder(seqs, dropout_rate)
    local_sen_embed = local_sen_embed.view(-1,
    ...
    extraction_mask_np = np.zeros_like(
        doc_mask_np).astype(np.bool) | doc_mask_np
    ...
    for step in range(valid_sen_idxs.shape[1]):
        ...
        if step > 0:
            extraction_context_embed = extraction_context_decoder(
                local_sen_embed, remaining_mask, extraction_mask, dropout_rate)
        p, p_stop, baseline = extractor(
            local_sen_embed, global_context_embed, extraction_context_embed, extraction_mask, dropout_rate)
        ...
        log_stop_prob = log_stop_prob.masked_fill(
            torch.logical_xor(done, just_stop), 0.0)
    ...
    if args.apply_length_normalization:
        log_prob_list = log_prob_list.sum(
            dim=1) / ((log_prob_list != 0).to(torch.float32).sum(dim=1))
    else:
...
def validation_iteration(batch):
    ...
    if pegasus_mode:
        doc_mask, sentences, summary = batch['doc_mask'], batch[
    ...
    local_sen_embed = local_sentence_encoder_ema(seqs)
    local_sen_embed = local_sen_embed.view(-1,
                                           num_sentences, local_sen_embed.size(1))
    ...
    for step in range(max_extracted_sentences_per_document):
        ...
        extraction_mask = torch.from_numpy(extraction_mask_np).to(device)
        if step > 0:
            extraction_context_embed = extraction_context_decoder_ema(
                local_sen_embed, remaining_mask, extraction_mask)
        p, p_stop, baseline = extractor_ema(
            local_sen_embed, global_context_embed, extraction_context_embed, extraction_mask)
    ...
    for doc_i in range(len(sentences)):
        ...
        extracted_sen_indices = np.argwhere(
            remaining_mask_np[doc_i] == False)[:, 0]
        hyp = "\n".join([sentences[doc_i][idx]
                        for idx in extracted_sen_indices]).strip()
...
for epoch in range(current_epoch, num_of_epochs):
    ...
    for count, batch in tqdm(enumerate(train_data_loader)):
        ...
        # Store intermediate loss values
        loss_train_list.append(loss)
        running_loss += loss

        # scheduler.step()

        update_moving_average(local_sentence_encoder_ema,
                              local_sentence_encoder, moving_average_decay)
        update_moving_average(global_context_encoder_ema,
                              global_context_encoder, moving_average_decay)
        update_moving_average(extraction_context_decoder_ema,
                              extraction_context_decoder, moving_average_decay)
        ...
        if current_batch % print_every == 0:
            ...
            LOG("[current_batch: %05d] loss: %.3f, learning rate: %f" % (
                current_batch, running_loss/print_every,  current_learning_rate))
            print("[current_batch: %05d] loss: %.3f, learning rate: %f" % (
                current_batch, running_loss/print_every, current_learning_rate))
        ...
        if (validate_every != 0 and current_batch % validate_every == 0) or count == len(train_data_loader) - 1:
            ...
            # validation
            val_score_list = []
            loss_train = sum(loss_train_list)/len(loss_train_list)
            wandb.log({"Intermediate Training Loss": loss_train})
            loss_train_list = []
            with torch.no_grad():
                for _, batch in tqdm(enumerate(val_data_loader)):
                    val_score_list += validation_iteration(batch)
                    # store the loss for val. values
                    # loss_test_list.append(val_score_list)
             

            # loss_test = sum(loss_test_list)/len(loss_test_list)
            # wandb.log({"Intermediate Testing Loss": loss_test})
            ...
            print("val: %.4f, %.4f, %.4f" %
                  (avg_val_rouge1, avg_val_rouge2, avg_val_rougeL))
            LOG("val: %.4f, %.4f, %.4f" %
                (avg_val_rouge1, avg_val_rouge2, avg_val_rougeL))
            wandb.log({"ROUGE-1 (Test)": avg_val_rouge1,
                       "ROUGE-2 (Test)": avg_val_rouge2,
                       "ROUGE-L (Test)": avg_val_rougeL})
